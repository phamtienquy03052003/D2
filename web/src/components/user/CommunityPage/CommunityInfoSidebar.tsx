...
const CommunityInfoSidebar: React.FC<CommunityInfoSidebarProps> = ({
  ...
  return (
    <div className="lg:col-span-1">
      <div className="rounded-xl p-4 bg-gray-100 dark:bg-[#1a1d25]">
        <div className="flex items-start justify-between mb-2 gap-2">
          <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 break-words flex-1 min-w-0">
            <CommunityName community={community} />
          </h2>
          {(isCreator || isModerator) && (
            <button
              onClick={() => setShowNameModal(true)}
              className="p-1.5 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-400 shrink-0 mt-0.5"
            >
              <Pencil size={16} />
            </button>
          )}
        ...
        <div className="flex items-start justify-between mb-4 gap-2">
          ...
          {(isCreator || isModerator) && (
            <button
              ...
              className="p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-400 shrink-0"
            >
              <Pencil size={14} />
            </button>
          )}
